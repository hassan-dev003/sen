...
bank transactions from the M2U transaction history printed to PDF (and later an API), drafts them
with a category, and holds
...
3. **Never lose the raw.** Every imported row keeps its original parsed text in `raw_rows`. The
   ...
   re-upload a file.
4. **Import is idempotent.** Importing the same capture twice produces zero new rows. Importing
...
- Dates: store `date` for booking dates (no time zone — a transaction line has a date, not an
...
The owner's real statements and captures are not in this repo and never will be. Do not ask for one to be
...
When you need to know how a real capture behaves in some edge case, ask a specific question
